// DS4 (DwarfStar) -- ROCm/HIP backend typed buffers.
//
// Mirrors `ds4-backend-cuda::buffers`. AMD's unified-memory model
// (HSA) means GPU + CPU share address space on Strix Halo, so the
// typed buffer + free-list design is identical to CUDA's, just
// hosted on a different runtime.
